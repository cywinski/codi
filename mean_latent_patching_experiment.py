# %%
"""
Mean Latent Patching Experiment: Testing the effect of patching output latent
embeddings with the mean latent vector from OTHER prompts at the same position.

This script first collects latent embeddings from all prompts, then for each prompt
patches specific positions with the mean embedding computed from all other prompts.
"""
...
def main():
    """Run the mean latent patching experiment."""
    load_dotenv()
    ...
    # Phase 1: Collect latent embeddings from all prompts
    print("\nPhase 1: Collecting latent embeddings from all prompts...")
    all_latents = []
    ...
    # Save results
    output_file = "mean_latent_patching_results.json"
...
def create_visualization(
    ...
    # Formatting
    ax.set_xlabel("Latent Position (Patched with Mean from Other Prompts)", fontsize=12)
    ax.set_ylabel("Accuracy Delta from Baseline", fontsize=14)
    ax.set_title(
        "Impact of Mean Latent Patching at Each Position\n"
    ...
    plt.tight_layout()
    plt.savefig(
        "mean_latent_patching_delta_by_position.png",
        dpi=300,
        bbox_inches="tight",
    )
    print("Saved: mean_latent_patching_delta_by_position.png")
    plt.close()
